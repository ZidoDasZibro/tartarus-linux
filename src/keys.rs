...
pub fn key_to_name(code: KeyCode) -> String {
    TABLE.iter().find(|(_, k)| *k == code).map(|(n, _)| n.to_string()).unwrap_or_else(|| format!("KEY_{code}"))
}

pub fn default_layer() -> [KeyCode; 20] {
    [KEY_1, KEY_2, KEY_3, KEY_4, KEY_5, KEY_6, KEY_7, KEY_8, KEY_9, KEY_0, KEY_A, KEY_B, KEY_C, KEY_D, KEY_E, KEY_F, KEY_G, KEY_H, KEY_I, KEY_J]
}

pub fn layer1_default() -> [KeyCode; 20] {
    [KEY_F1, KEY_F2, KEY_F3, KEY_F4, KEY_F5, KEY_F6, KEY_F7, KEY_F8, KEY_F9, KEY_F10, KEY_F11, KEY_F12, KEY_HOME, KEY_END, KEY_PAGEUP, KEY_PAGEDOWN, KEY_INSERT, KEY_DELETE, KEY_UP, KEY_DOWN]
}
