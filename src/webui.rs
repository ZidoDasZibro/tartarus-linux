...
function keyCard(i,k){
  ...
  return `<div class="kcard ${k.mode==='axis'?'axis':''}" data-i="${i}">
    ...
    <div class="modline">
      ...
      <label class="inline"><input type="checkbox" class="mods_on" ${k.mods_on||(k.lctrl||k.rctrl||k.lshift||k.rshift||k.lalt||k.ralt||k.lwin||k.rwin&&!k.mods_on_full)?'checked':''}> on normal</label>
...
function collectKeys(){
  return [...document.querySelectorAll('.kcard')].map(card=>{
    ...
    // Flags list selected mods; mods_on / mods_on_full say where they apply
